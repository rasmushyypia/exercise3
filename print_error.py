...
class TurtleBot(Node):
    ...
    def __init__(self):
        ...
        self.error_publisher = self.create_publisher(Point, '/goal_error', 10)
    ...
    def move2goal(self):
        ...
        while rclpy.ok():  # Keep running until ROS is shutdown
            ...
            while self.euclidean_distance(goal_pose) >= distance_tolerance:
                ...
                # Calculate and publish the error
                error_msg.x = abs(goal_pose.x - self.pose.x)
                error_msg.y = abs(goal_pose.y - self.pose.y)
                orientation_error = atan2(sin(goal_theta - self.theta), cos(goal_theta - self.theta))
                error_msg.z = abs(orientation_error)
